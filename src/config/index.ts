...
const PROTOCOL = IS_LOCAL
  ? 'http'
  : IS_DEV && process.env.STORAGE_NODE_VERSION == STORAGE_NODE_VERSION.BETA
  ? 'https'
  : 'https';
...
export const PUBLIC_SHARE_URL = IS_LOCAL
  ? 'http://localhost:3080'
  : process.env.STORAGE_NODE_VERSION == STORAGE_NODE_VERSION.BETA
...
export const DEFAULT_STORAGE_NODE_IP = IS_LOCAL
  ? 'localhost'
  : IS_DEV
